...
function iniciar() {
    ...
    client.addEventListener("open", function (m) {
      ...
      sendMessageClient({tipo: "GET_CLIP_HORA", dados: {camera: "camera_3", startTime: "1685376225", endTime: "1685379941"}});
    });
    
    client.addEventListener("message", function (m) {
      log(m);

      log(m.data.constructor.name);
      log(m.data.name);

      if (m.data instanceof Blob) {
        log("eh instancia");
        video.src = window.URL.createObjectURL(m.data);
      }
    });
}
